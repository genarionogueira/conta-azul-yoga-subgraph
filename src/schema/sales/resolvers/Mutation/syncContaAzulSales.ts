...
import type { SyncResult } from '../../../../lib/sale-sync/types.js'
...
export async function syncContaAzulSales(
  ...
  if (storeId) {
    const result = await saleSyncService.syncStore(tenantId, storeId, 'manual')
    return mapStoreSyncResult(result)
  }

  const result = await saleSyncService.reconcileAll('manual')
